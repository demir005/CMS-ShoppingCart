...
using PagedList;
using CMSShoppingCard.Areas.Admin.Models.ViewModels.Shop;
        ...
        public ActionResult Orders()
        {
            ...
            using (Db db = new Db())
            {
                ...
                foreach(var order in orders)
                {
                    ...
                    List<OrderDetailsDTO> orderDetaiList = db.OrderDetails.Where(x => x.OrderId == order.OrderId).ToList();

                    UserDTO user = db.Users.Where(x => x.Id == order.UserId).FirstOrDefault();

                    foreach(var orderDetails in orderDetaiList)
                    {
